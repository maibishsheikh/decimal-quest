...
// NOTE: font sizes across this entire Play phase are intentionally larger
// than the Story phase (per module spec) — Simulate & Play are the two
// phases where text and touch targets are bumped up for readability and
// smoother, easier-to-tap navigation through the games.
function StarRow({ stars }) {
  return (
    <div className="star-rating" style={{ fontSize: '1.4rem', margin: 0 }}>
...
function WorldSelector({ onSelect, worldProgress }) {
  return (
    <div className="play-screen">
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.7rem', textAlign: 'center', flexShrink: 0 }}>
        Choose Your World 🌍
      </h2>
...
function ModeSelector({ world, onSelect, onBack }) {
  return (
    <div className="play-screen">
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: 'var(--color-text-muted)', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1rem', alignSelf: 'flex-start', flexShrink: 0 }}>← Back</button>
  ...
  return (
    <div className="play-screen">
      <motion.div className="glass-card"
        ...
        <div style={{ fontSize: '2.9rem', marginBottom: 6 }}>{world.emoji}</div>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.75rem', color: 'var(--gold)', marginBottom: 6 }}>
          {world.name} Complete!
        </h2>
        <StarRow stars={stars} />
        <p style={{ margin: '10px 0', fontSize: '1.15rem' }}>
          {result.correct} / {result.total} correct · {result.xp} XP
        </p>
        {result.boss && !result.lost && (
          <p style={{ color: 'var(--gold)', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.15rem', marginBottom: 4 }}>
...
export default function PlayPhase({ onComplete, playNarration }) {
  ...
  const handleModeSelect = m => {
    setSelectedMode(m);
    setQuestions(generateModeQuestions(selectedWorld.id, m.questionCount, { excludeHard: m.id === 'guided', preferBoss: m.id === 'boss' }));
    setScreen('playing');
  };
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      {screen === 'playing' && (
        <motion.div key="playing" className="play-gameplay" style={{ position: 'absolute', inset: 0, top: 64, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 14px', overflowY: 'auto', overflowX: 'hidden' }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          {/* Breadcrumb */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, width: '100%', maxWidth: 600, flexShrink: 0, fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.95rem', color: 'var(--color-text-muted)' }}>
            <button onClick={() => setScreen('modes')} style={{ background: 'none', border: 'none', color: 'var(--color-text-muted)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 'inherit', fontSize: 'inherit' }}>
