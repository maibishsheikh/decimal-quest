...
const TAKEAWAYS = [
  { icon: '🔟', text: 'A fraction with denominator 10 slides straight into the tenths place: N/10 = 0.N.' },
  { icon: '🎨', text: 'A fraction with denominator 100 becomes hundredths — always two digits after the point.' },
  { icon: '🧩', text: 'The same amount can be written as different fractions, but they share one decimal.' },
  { icon: '✔️', text: 'Compare decimals by checking the tenths digit first, then the hundredths digit.' },
];

const CONFIDENCE_OPTIONS = [
  { emoji: '🌟', text: "I'm great at converting fractions!" },
  { emoji: '👍', text: 'I know most tenths and hundredths!' },
    ...
    return (
      <div className="reflect-screen" style={{ justifyContent: 'center' }}>
        <div className="reflect-card glass-card" style={{ width: '100%', maxWidth: 520, textAlign: 'center' }}>
          ...
          <p style={{ fontWeight: 700, marginBottom: 16 }}>How confident do you feel about converting fractions to decimals?</p>
      ...
      <div style={{ width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 16 }}>
        ...
        <div className="reflect-card glass-card" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem', marginTop: 6 }}>
            {pct >= 80 ? "Outstanding! You're a Decimal Master! 🏆" : pct >= 60 ? "Well done! Keep practising! 💪" : "Review the key facts below 📚"}
          </p>
        </div>
        ...
        <div className="conversion-fact-box">
          <span className="mass-value">N/10 = 0.N  •  N/100 = 0.0N</span>
          <div style={{ fontSize: '0.85rem', marginTop: 4, color: 'rgba(255,255,255,0.65)' }}>
            Tenths = 1 digit after the point, hundredths = 2 digits
          </div>
        </div>
        ...
        <div className="reflect-badge glass-card">
          ...
          <div>
            ...
            <div className="reflect-badge-sub">Decimals Using Convert Fractions</div>
          </div>
        </div>
